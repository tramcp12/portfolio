...
> **src/ is in Phase 4 (all splits complete).** CSS, HTML, JS, and data are assembled from `src/` by `build.js`. Room data lives in `src/data/rooms.json` and is injected by the build as `<script id="rooms-data">` for IIFE 0. Travel and journal data live in `src/data/travel.json` / `src/data/journal.json` as reference files (their static HTML partials are retained for SEO).
